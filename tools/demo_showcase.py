...
OUT_DIR = "tools/demo_showcase"
os.makedirs(OUT_DIR, exist_ok=True)

async def generate(session, prompt_idx, category, prompt, model, sem):
    fname = f"{OUT_DIR}/{category}_{prompt_idx:02d}_{model}.json"
    ...
    async with sem:
        payload = {
            "prompt": prompt,
            "player_id": f"demo_{model}_{prompt_idx}",
            "category": category,
            "style": "freestyle",
            "model": model,
            "dm_mode": True,
            ...
            try:
                async with session.post(
                    ...
                    if data.get("success") and data.get("model"):
                        ...
                        print(f"  OK  {model:15s} {category:8s} #{prompt_idx:02d} ({parts} parts) - {prompt[:40]}")
                        return "ok"
                    else:
...
async def main():
    ...
    print(f"Generating {total} showcase models (15 prompts x 4 models)...")
    start = time.time()
